...
import { 
  Send, X, Phone, User, Sparkles, 
  ChevronDown, Clock, Mail 
} from "lucide-react";
          ...
          >
            <div className="pointer-events-auto bg-[#041a14]/95 backdrop-blur-md text-white p-2 rounded-full shadow-2xl border border-white/10 flex items-center gap-2 max-w-6xl w-full mx-6 transition-all duration-500 hover:scale-[1.01]">
              ...
              <form onSubmit={handleVerify} className="flex-none flex items-center gap-2 px-2 ">
                ...
                <div className="h-10 px-4 rounded-full bg-white/5 border border-white/10 flex items-center w-40 focus-within:bg-white/10 transition-colors relative shrink-0">
              ...
              <div className="p-6 -mt-4 bg-white rounded-t-[2rem] relative z-10 space-y-4 pb-12">
                 <form onSubmit={handleVerify} className="space-y-4">
                   ...
                   <div className="relative">
                      ...
                      <input type="tel" placeholder="Phone Number" required className="w-full bg-gray-50 border border-gray-200 rounded-xl pl-11 py-3.5 text-sm outline-none focus:border-[#F36F21]" onChange={(e) => setFormData({...formData, phone: e.target.value})} />
                   </div>
                   ...
                   <button type="submit" disabled={loading} className="w-full py-4 rounded-xl bg-[#F36F21] text-white font-bold uppercase tracking-wide shadow-lg disabled:opacity-50">{loading ? "Processing..." : "Confirm Request"}</button>
                 </form>
              </div>
            </motion.div>
          </div>
